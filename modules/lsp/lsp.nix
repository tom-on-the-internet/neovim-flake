...
in {
  options.vim.lsp = {
    ...
    capabilities = mkOption {
      type = types.lines;
      description =
        "LSP capabilities: A function that takes `capabilities` and returns that variable";
    };
    luaLocals = mkOption {
      type = types.lines;
      description =
        "Lua statements that have access to capabilities & on_attach and are accessible in the servers";
    };
    onAttach = mkOption {
      type = types.lines;
    ...
    lightbulb = mkOption {
      type = types.bool;
      description =
        "Enable nvim-lightbulb: show a lightbulb when a code action is availaible";
    };
    ...
    makeServer = name: options:
      "${writeIf options.enable ''
          require("lspconfig")["${name}"].setup({
            ...
            capabilities = ${
              if options.capabilities != null then
      ...
      ''}";
    makeServers = servers:
      map (server: makeServer server (getAttr server servers))
      (attrNames servers);
  in mkIf cfg.enable {
    vim.startPlugins = with pkgs.neovimPlugins; [
      ...
      (if cfg.lspLoading.enable then fidget else null)
    ];

    vim.configRC = ''
      ${writeIf cfg.lightbulb
      "autocmd CursorHold,CursorHoldI * lua require'nvim-lightbulb'.update_lightbulb()"}
      ${writeIf cfg.diagnosticsPopup
      "autocmd CursorHold * lua vim.diagnostic.open_float()"}
    '';

    vim.luaConfigRC = let
      nullLsSources = sources:
        concatStringsSep "," (map (source: "null_ls.${source}") sources);
      disabledClientsCheck = if cfg.format.disabledClients == [ ] then
        "true"
      else
        concatStringsSep " and " (map (client: ''client.name ~= "${client}"'')
          cfg.format.disabledClients);
    in ''
